...
class ModeloAdministradores{
    ...
    {
        $stmt = Conexion::conectar()->prepare("INSERT INTO $tabla(nombre, usuario, contrasena,  perfil, estado) VALUES (:nombre, :usuario, :contrasena, :perfil, :estado)");
    ...
    static public function mdlEditarAdministrador($tabla, $datos){
		$stmt = Conexion::conectar()->prepare("UPDATE $tabla SET nombre = :nombre, usuario = :usuario, contrasena = :contrasena,  perfil = :perfil WHERE idAdministrador = :idAdministrador");

		$stmt->bindParam(":nombre", $datos["nombre"], PDO::PARAM_STR);
		$stmt->bindParam(":usuario", $datos["usuario"], PDO::PARAM_STR);
		$stmt->bindParam(":contrasena", $datos["contrasena"], PDO::PARAM_STR);
		$stmt->bindParam(":perfil", $datos["perfil"], PDO::PARAM_STR);
		$stmt->bindParam(":idAdministrador", $datos["idAdministrador"], PDO::PARAM_INT);
